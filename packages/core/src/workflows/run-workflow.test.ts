...
describe("runWorkflow", () => {
  it("runs all workflow steps and marks the run as succeeded", async () => {
    ...
    const updatedRun = await workflowRunRepository.getWorkflowRunById(workflowRun.id);
    const steps = await workflowRunRepository.listWorkflowStepsByRunId(workflowRun.id);
  ...
  it("marks the run as failed when retries are exhausted", async () => {
    ...
    const failingStep = vi.fn().mockRejectedValue(new Error("permanent failure"));
