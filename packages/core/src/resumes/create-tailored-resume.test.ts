  ...
  return [
    {
      ...
      suggestedContent: "Backend engineer with API, platform, and fintech integration experience.",
...
describe("createCreateTailoredResume", () => {
  it("creates a separate tailored profile and leaves the source profile unchanged", async () => {
    ...
    const sourceVersions = await resumeVersions.listResumeVersionsByProfileId("profile-1");
    const tailoredVersions = await resumeVersions.listResumeVersionsByProfileId(result.profile.id);
  ...
  it("creates a new tailored profile on repeated runs instead of mutating the source", async () => {
    ...
    const sourceProfile = await resumeProfiles.getResumeProfileById("profile-1");
    const sourceVersions = await resumeVersions.listResumeVersionsByProfileId("profile-1");
    const firstTailoredVersions = await resumeVersions.listResumeVersionsByProfileId(
      first.profile.id,
    );
    const secondTailoredVersions = await resumeVersions.listResumeVersionsByProfileId(
      second.profile.id,
    );
  ...
  it("throws when generated tailoring suggestions are malformed", async () => {
    ...
    const versions = await resumeVersions.listResumeVersionsByProfileId("profile-1");
