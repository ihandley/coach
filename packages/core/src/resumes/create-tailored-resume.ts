import type { ResumeProfile, ResumeVersion, TailoredResume, TailoringSuggestion } from "./types";
...
function isTailoringSuggestion(value: unknown): value is TailoringSuggestion {
  ...
  return (
    ...
    candidate.relatedJobRequirements.every((requirement) => typeof requirement === "string") &&
...
export function createCreateTailoredResume({
  ...
  return async function createTailoredResume(input: {
    ...
    const sourceVersion = await resumeVersions.getResumeVersionById(input.sourceResumeVersionId);
