import type { NormalizedResume, ResumeSource, ResumeVersion } from "./types";
...
export function createInMemoryResumeVersionRepository() {
  ...
  return {
    ...
    async listResumeVersionsByProfileId(profileId: string): Promise<ResumeVersion[]> {
      return versions
        .filter((version) => version.profileId === profileId)
        .sort((a, b) => a.versionNumber - b.versionNumber);
    },

    async getResumeVersionById(resumeVersionId: string): Promise<ResumeVersion | null> {
      return versions.find((version) => version.id === resumeVersionId) ?? null;
    },
  };
}
