...
describe("JobImporter.importJobFromUrl", () => {
  it("rejects an invalid URL before attempting any external work", async () => {
    ...
    const saveImportedJob = async (_input: {
      company: string;
      title: string;
  ...
  it("rejects a non-http URL", async () => {
    ...
    await expect(() => importer.importJobFromUrl("ftp://example.com/job/123")).rejects.toThrow(
      InvalidJobImportUrlError,
    );
  });
  ...
  it("rejects malformed extracted job data before save", async () => {
    ...
    await expect(() => importer.importJobFromUrl("https://example.com/jobs/123")).rejects.toThrow(
      InvalidExtractedJobDataError,
    );
