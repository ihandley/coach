...
function isRecommendationCategory(value: unknown): value is RecommendationCategory {
  return (
    typeof value === "string" && RECOMMENDATION_CATEGORIES.includes(value as RecommendationCategory)
  );
}
...
export function createJobFitScorer({
  ...
  return {
    async scoreJobFit({ jobId, resumeProfileId }: ScoreJobFitInput) {
      ...
      const resumeProfile = await resumeProfiles.getResumeProfileById(resumeProfileId);
      ...
      if (!evaluate) {
        throw new JobFitScorerError("NOT_IMPLEMENTED", "Scoring not implemented yet");
      }
