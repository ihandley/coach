...
async function getDefaultResumeText(db: SupabaseClient) {
...
export async function persistJobMatchForDefaultResume(db: SupabaseClient, job: MatchableJob) {
  const { resumeProfileId, resumeText } = await getDefaultResumeText(db);
  ...
  const { error } = await db.from("job_matches").upsert(row);
