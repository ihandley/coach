...
describe("createGenerateApplicationMaterialsWorkflow", () => {
  ...
  it("records a failed run when packet export fails", async () => {
    ...
    const exportApplicationPacket = vi.fn().mockRejectedValue(new Error("EXPORT_PACKET_FAILED"));
    ...
    const steps = await workflowRunRepository.listWorkflowStepsByRunId(runs[0].id);
