  ...
  const experience = Array.isArray(normalizedResume.experience)
    ? normalizedResume.experience
        .map((item) => {
          ...
          const bullets = [...getStringArray(record.bullets), ...getStringArray(record.highlights)];
        ...
        .filter(
          (item) => item.company.length > 0 || item.title.length > 0 || item.bullets.length > 0,
        )
    : [];
...
export function createExportsServer(dependencies?: {
  ...
  const resumeProfiles = dependencies?.resumeProfiles ?? createSupabaseResumeProfileRepository(db!);

  const resumeVersions = dependencies?.resumeVersions ?? createSupabaseResumeVersionRepository(db!);

  const coverLetters = dependencies?.coverLetters ?? createSupabaseCoverLetterRepository(db!);

  const artifacts = dependencies?.artifacts ?? createSupabaseExportedArtifactRepository(db!);

  const renderResumeDocxImpl = dependencies?.renderResumeDocx ?? renderResumeDocx;

  const renderResumePdfImpl = dependencies?.renderResumePdf ?? renderResumePdf;
  ...
  return {
    exportDocument: createExport({
      ...
      exportApplicationPacket: ({ format, resumeProfileId, resumeVersionId, coverLetterDraftId }) =>
