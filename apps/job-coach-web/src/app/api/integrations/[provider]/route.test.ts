...
describe("GET /api/integrations/[provider]", () => {
  it("returns integration account state", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/integrations/gmail"), {
  ...
  it("returns 400 for unsupported providers", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/integrations/not-real"), {
