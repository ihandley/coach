...
function isWorkflowType(
  ...
  return value === "import-job-and-score-fit" || value === "generate-application-materials";
}
...
export async function POST(request: Request) {
  ...
  try {
    if (body.workflowType === "import-job-and-score-fit") {
      if (!isNonEmptyString(body.sourceUrl) || !isNonEmptyString(body.resumeProfileId)) {
        return Response.json({ error: "INVALID_WORKFLOW_INPUT" }, { status: 400 });
      }

      const result = await workflowsServer.startImportJobAndScoreFitWorkflow({
    ...
    const result = await workflowsServer.startGenerateApplicationMaterialsWorkflow({
      resumeProfileId: body.resumeProfileId,
