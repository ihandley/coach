...
vi.mock("../../../server/workflows-server", () => {
  return {
    workflowsServer: {
      ...
      startImportJobAndScoreFitWorkflow: startImportJobAndScoreFitWorkflowMock,
      startGenerateApplicationMaterialsWorkflow: startGenerateApplicationMaterialsWorkflowMock,
    },
  };
});
...
describe("POST /api/workflows", () => {
  ...
  it("starts generate-application-materials workflow", async () => {
    ...
    expect(startGenerateApplicationMaterialsWorkflowMock).toHaveBeenCalledWith({
      resumeProfileId: "rp1",
      resumeVersionId: "rv1",
      jobId: "job1",
      format: "pdf",
  ...
  it("returns 400 for invalid input", async () => {
    ...
    expect(startGenerateApplicationMaterialsWorkflowMock).not.toHaveBeenCalled();
  });

  it("returns 500 when workflow execution fails", async () => {
    const { POST } = await import("./route");

    startImportJobAndScoreFitWorkflowMock.mockRejectedValue(new Error("SCORING_UNAVAILABLE"));
