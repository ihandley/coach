...
import { createServerClient, DbJobRepository, createDbResumeProfileRepository, createDbResumeVersionRepository } from "@coach/db";
...
export async function GET() {
    ...
    if (resumeProfiles.length > 0) {
        ...
        if (profile.currentVersionId) {
            try {
                const currentVersion = await resumeVersionRepo.getResumeVersionById(profile.currentVersionId);
                if (currentVersion) {
                    resumeText = normalizedResumeToText(currentVersion.normalizedResume);
                }
            } catch (error) {
